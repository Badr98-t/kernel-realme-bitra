    ...
    case $COMPILER in
        proton)
            if [ $COMPILER_CLEANUP = true ]; then
                rm -rf ~/kernel/neutron-clang
            fi
            if [ $(ls $HOME/kernel/proton-clang 2>/dev/null | wc -l) -ne 0 ]; then
                PATH="$HOME/kernel/proton-clang/bin:$PATH"
            else
                git clone --depth=1  https://github.com/kdrag0n/proton-clang.git ~/kernel/proton-clang
                PATH="$HOME/kernel/proton-clang/bin:$PATH"
            fi
            ;;
        neutron)
            if [ $COMPILER_CLEANUP = true ]; then
                rm -rf ~/kernel/proton-clang
            fi
            if [ $(ls $HOME/kernel/neutron-clang/bin 2>/dev/null | wc -l ) -ne 0 ] && 
               [ $(find $HOME/kernel/neutron-clang -name *.tar.zst | wc -l) -eq 0 ]; then
                PATH="$HOME/kernel/neutron-clang/bin:$PATH"
            else
                rm -rf ~/kernel/neutron-clang
                mkdir -p ~/kernel/neutron-clang
                cd ~/kernel/neutron-clang || exit
